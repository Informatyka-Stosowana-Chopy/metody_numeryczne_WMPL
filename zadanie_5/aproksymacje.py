from wzory import wartosc_funkcji
...
def wartosc_licznika(wybor_funkcji, liczba_wezlow, k):
    """
...
def blad_aproksymacji(wybor_funkcji, k, tab_wsp, liczba_wezlow):
    """
    :param wybor_funkcji: -//-
    :param k: stopień wielomianu aproksymującego (int)
    ...
    for i in range(liczba_wezlow):
        ...
        # x = - 1 + 2 * (x - a)/(b - a)
        w = (wspolczynniki(liczba_wezlow, i)[1])
        calka += w * (wartosc_funkcji(x, wybor_funkcji) - wartosc_wielomianu(k, x, tab_wsp)) ** 2
    return calka


def wspolczynnik_aproksymujacy(wybor_funkcji, liczba_wezlow, k):
    """
    :param wybor_funkcji: -//-
    :param liczba_wezlow: -//-
    :param k: -//-
    ...
    """
    wsp = (2 * k + 1) / 2 * wartosc_licznika(wybor_funkcji, liczba_wezlow, k)
    return wsp


def lista_wspolczynnikow(wybor_funkcji, liczba_wezlow, k):
    """
    :param wybor_funkcji: -//-
    :param liczba_wezlow: -//-
    :param k: -//-
    ...
    wielomian = []
    for i in range(k + 1):
        wielomian.append(wspolczynnik_aproksymujacy(wybor_funkcji, liczba_wezlow, i))
    return wielomian
